...
# 调用后端脚本接口
def test_popen(request):
	while True:
		nextline=s.stdout.readline()
		print(nextline.strip())
		if nextline=="" and scan.poll()!=None:
			break
# 查看后端脚本运行,并输出到前端,但没用websocket,因此不能实时查看,需要手动刷新查看
def test_out(request):
	return HttpResponse(s.stdout.readline())
...
def WebSocket(request):
	if request.is_websocket():
		...
		# while 1:
		# 	if message:
		# 		request.websocket.send('大鱼'.encode())  # 发送给前端的数据
		# 		# time.sleep(2)
		# 	else:
		# 		print(232222222222)
		...
		for i in range(100):
			print("message:",message)
			request.websocket.send('大鱼'.encode('utf8'))  # 发送给前端的数据
		print("循环结束")
		request.websocket.close()


# tensorflow的word2vec算法
# Training Parameters
learning_rate = 0.001
			...
			for step in range(1, training_steps+1):
				...
				if step % display_step == 0 or step == 1:
					...
					print("Step " + str(step) + ", Minibatch Loss= " + \
							...
							"{:.3f}".format(acc))
					request.websocket.send("Step " + str(step) + ", Minibatch Loss= " + "{:.4f}".format(loss) + ", Training Accuracy= " + "{:.3f}".format(acc))  # 发送给前端的数据
